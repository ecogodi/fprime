...
describe('F -> ', function(){
    ...
    describe('"result" factory helper tests -> ', function(){
        ...
        it('sync function error', function(done){
            var syncFunc = function(){ throw 'foo error'; };
    ...
    describe('parallel "onErrorExit" step helper tests -> ', function(){
        it('parallel error exit', function(done){
            F(pe,
                ...
                assert.notOk(err[0],'no error');
                assert.equal(err[1],'error_message', 'expected error');
                assert.notOk(result,'expected null result');
            });
        });

        it('parallel (queued) error continue', function(done){
            F( {0:a,1:b},
                e,[c,d]
            )('start',function(err,resulta,resultb){
                done();
                assert.isObject(err,'array-like error');
                assert.notOk(err[0],'no error');
                assert.notOk(err[1],'no error');
                assert.equal(resulta,'start-a-c', 'expected result');
                assert.equal(resultb,'start-b-d', 'expected result');
            });
        });

        it('parallel (mapped) error continue', function(done){
            F( {a:a,b:b},
                e,[c]
            )('start',function(err,result){
                done();
                ...
                assert.isObject(result,'array-like result');
                assert.equal(result['a'],'start-a-c', 'expected result');
                assert.equal(result['b'],'start-b-c', 'expected result');
            });
        });
    });
